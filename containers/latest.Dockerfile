...
# 2. chainsformer-spark-source-1.0-latest-shaded.jar, only if the active branch is master.
RUN export BRANCH=${BUILDKITE_BRANCH} \
    ...
    && if [ "${BRANCH}" = "master" ] || [ "${BRANCH}" = "origin/master" ]; then \
        export LATEST_TARGET=chainsformer-spark-source-1.0-latest-shaded.jar \
        && cp target/${TARGET} assets/${LATEST_TARGET} \
        && echo "ASSETS: ${LATEST_TARGET}"; \
    fi \
